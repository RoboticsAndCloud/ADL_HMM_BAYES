...
ACTION_DICT = {
    0: ["robot_camera"],  
    1: ["robot_mic"], 
    2: ["robot_camera", "robot_mic"], 
    3: ["wmu_camera"], 
    4: ["wmu_mic"], 
    5: ["wmu_camera", "wmu_mic"], 
    6: ["robot_camera", "wmu_camera"], 
    7: ["robot_camera", "wmu_mic"], 
    8: ["robot_camera", "robot_mic", "wmu_camera", "wmu_mic"],
}

DEFAULT_ACTION = 8
SENSOR_TYPE_ROBOT = 'robot'
SENSOR_TYPE_WMU = 'wmu'
...
def get_object_by_activity_yolo(time_str, activity, sensor_type):
    """
    # Location
    ...
    object_dict = tools_ascc_bandit.get_activity_by_vision_yolov3(time_str, action='vision', mode='None-map', activity = activity, sensor_type = sensor_type)
    print('get_object_by_activity_yolo time_str:', time_str, ' object:', object_dict)

    return object_dict

# get location by CNN model
def get_location_by_activity_cnn(time_str, activity, sensor_type):
    """
    # Location
    LOCATION_READINGROOM = 'readingroom'
    LOCATION_BATHROOM = 'bathroom'
    ...
    # /home/ascc/LF_Workspace/Motion-Trigered-Activity/home_room_classification/keras-image-room-clasification/src/
    # ascc_room_activity_test.py
    location, prob, image_dir = tools_ascc_bandit.get_activity_by_vision_dnn(time_str, action='vision', activity = activity, sensor_type = sensor_type)
...
def get_motion_type_by_activity_cnn(time_str, activity, sensor_type):

    # Mapping
    # should be act : probability
    # /home/ascc/LF_Workspace/Motion-Trigered-Activity/home_room_classification/keras-image-room-clasification/src/
    # ascc_room_activity_test.py
    motion_type_list, prob_list = tools_ascc_bandit.get_activity_by_motion_dnn(time_str, action='vision', activity = activity, sensor_type = sensor_type)
    motion_type = ''
    prob = -1
...
def get_audio_type_by_activity_cnn(time_str, activity, sensor_type):
    # audio type:
    # door_open_closed
    # drinking
    # eating
    ...
    # /home/ascc/LF_Workspace/Motion-Trigered-Activity/home_room_classification/keras-image-room-clasification/src/
    # ascc_room_activity_test.py
    audio_type, prob = tools_ascc_bandit.get_activity_by_audio_dnn(time_str, action='vision', activity = activity, sensor_type = sensor_type)
...
class EnvBanditASCC():
    def __init__(self, time_str = '2009-12-11 08:00:00', act='kitchen'):
        
        high = np.array([1.0, 1.0, BATTERY_LIFE], dtype=np.float32)

        self.activity_folder = act
    ...
    def reset(self):
        ...
        next_state_ = self.get_context_by_action(ACTION_DICT[DEFAULT_ACTION], time_str)
    ...
    # 8: ["robot_camera", "robot_mic", "wmu_camera", "wmu_mic"]
    def get_context_by_action(self, action_str_list, time_str):
        location = ''
        object = ''
        sound = ''
        motion = ''
        ...
        for sensor in action_str_list:
            sensor_type = sensor.split('_')[0]
            data_type = sensor.split('_')[1]
            if data_type == 'camera':
                location = get_location_by_activity_cnn(time_str, self.activity_folder, sensor_type)
                object = ''



        features = [location, object, sound, motion, person_status, distance]
        return features
